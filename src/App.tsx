...
function FloatingPetals({ disabled = false }: { disabled?: boolean }) {
  ...
  useEffect(() => {
    ...
    const colors = ["#f1dfb8", "#e7c784", "#deaf55", "#f9f1e1"];
  ...
  return (
    <div className={`pointer-events-none fixed inset-0 overflow-hidden z-40 ${isLowPowerMode ? "opacity-70" : ""}`}>
      {petals.map((petal) => (
        <motion.div
          key={petal.id}
          className="absolute drop-shadow-[0_2px_6px_rgba(212,149,46,0.5)]"
...
function CountdownTimer() {
  const targetDate = new Date("December 14, 2026 18:30:00").getTime();
...
export default function WeddingInvitation() {
  ...
  // Form states
  const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz0eG7BYmr6-O2lDULcsNglLgEwhYDRYVeX_c8xD90027TFzOk8iq0JmXPfKZsFTTQp/exec";

  const [rsvpData, setRsvpData] = useState({ fullName: '', guests: '1', dietaryNotes: '' });
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRsvpData({ fullName: '', guests: '1', dietaryNotes: '' });
      } else {
        setRsvpStatus('error');
      }
    } catch (error) {
  ...
  const [wishesData, setWishesData] = useState({ name: '', message: '' });
  const [wishesStatus, setWishesStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');

  const handleWishesSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setWishesStatus('loading');
    try {
      const response = await fetch(SCRIPT_URL, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ type: 'wishes', ...wishesData }),
      });
      if (response.ok) {
        setWishesStatus('success');
        setWishesData({ name: '', message: '' });
      } else {
  ...
  return (
    ...
    >
      ...
      <audio ref={audioRef} src="/background song .mp3" loop />
        ...
        {!isOpened ? (
          ...
          >
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
              ...
              <p className="text-stone-500 text-sm tracking-[0.2em] font-light">14 DECEMBER 2026</p>
            </motion.div>
            ...
            >
              <div className="absolute -inset-8 bg-[radial-gradient(circle,_rgba(212,149,46,0.35)_0%,_rgba(241,223,184,0.2)_45%,_transparent_75%)] blur-3xl opacity-90" />
              <div className="absolute inset-0 bg-gradient-to-b from-[#fffefb] via-[#fff9f2] to-[#fff6ee] rounded-[1.4rem] shadow-[0_28px_80px_-20px_rgba(101,59,27,0.35)] border border-theme-200/80 overflow-hidden" />
              ...
              {/* Left Flap */}
              <motion.div
                className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-br from-[#c89945] via-[#dfbb73] to-[#ab7c31] z-30 shadow-[8px_0_28px_rgba(139,103,45,0.45)] origin-left flex items-center justify-end pr-4 overflow-hidden rounded-l-[1.2rem]"
              ...
              >
                ...
                <InviteImage
                  src={mandalaImage}
                  className="absolute -top-20 md:-top-28 -left-20 md:-left-28 w-56 md:w-72 h-56 md:h-72 opacity-40 mix-blend-multiply"
                  alt=""
                />
                <InviteImage
                  src={mandalaImage}
                  className="absolute -bottom-20 md:-bottom-28 -left-20 md:-left-28 w-56 md:w-72 h-56 md:h-72 opacity-40 mix-blend-multiply -rotate-90"
                  alt=""
                />

                <div className="text-white/60 rotate-90 whitespace-nowrap text-xs tracking-[0.55em] uppercase font-bold relative z-10">
                  NIMESH & DHANANJI
                </div>
              </motion.div>
              ...
              {/* Right Flap */}
              <motion.div
                className="absolute inset-y-0 right-0 w-1/2 bg-gradient-to-bl from-[#c89945] via-[#dfbb73] to-[#ab7c31] z-30 shadow-[-8px_0_28px_rgba(139,103,45,0.45)] origin-right flex items-center justify-start pl-4 overflow-hidden rounded-r-[1.2rem]"
                whileHover={{ rotateY: 14 }}
                transition={{ type: "spring", stiffness: 110, damping: 16 }}
              >
                <div className="absolute inset-0 opacity-20 paper-grain" />
                ...
                <InviteImage
                  src={mandalaImage}
                  className="absolute -top-20 md:-top-28 -right-20 md:-right-28 w-56 md:w-72 h-56 md:h-72 opacity-40 mix-blend-multiply rotate-90"
                  alt=""
                />
                <InviteImage
                  src={mandalaImage}
                  className="absolute -bottom-20 md:-bottom-28 -right-20 md:-right-28 w-56 md:w-72 h-56 md:h-72 opacity-40 mix-blend-multiply rotate-180"
                  alt=""
                />
              </motion.div>
              ...
              <motion.div
                ...
                whileTap={{ scale: 0.9 }}
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-40 w-28 h-28 rounded-full bg-gradient-to-br from-theme-200 via-[#fffbf5] to-theme-300 shadow-[0_20px_45px_-10px_rgba(184,140,66,0.65)] border-[5px] border-[#a67c33] flex items-center justify-center group-hover:shadow-theme-500/40"
              >
                ...
                <div className="text-center relative z-10">
                  <p className="font-cinzel text-[1.7rem] font-bold text-stone-800 leading-none">N&D</p>
                ...
                <div className="flex flex-col items-center text-center space-y-4 flex-1 w-full relative z-10">
                  ...
                  >
                    ...
                    <div className="font-cinzel space-y-1">
                      <p className="text-sm md:text-base text-stone-700 tracking-[0.2em] md:tracking-[0.3em] font-bold">14 DECEMBER 2026</p>
                      <p className="text-[8px] md:text-[9px] text-theme-600 tracking-[0.2em] uppercase font-bold">Moratuwa, Sri Lanka</p>
                    </div>
                  </motion.div>
                </div>
              </motion.div>
            ...
            <section className="cv-auto py-24 md:py-32 w-full flex flex-col items-center px-4 relative">
              ...
              <div className="max-w-[1000px] w-full flex flex-col items-center text-center relative z-10">
                ...
                >
                  ...
                  <div className="relative bg-white/90 p-2.5 md:p-3 rounded-[2rem] border border-theme-200 shadow-[0_20px_50px_-20px_rgba(124,72,30,0.45)]">
                    <InviteImage
                      src={brideGroomImage}
                ...
                <div className="relative w-full flex flex-col md:flex-row items-center justify-center md:items-stretch gap-6 md:gap-10 my-12 md:my-20 z-10 px-2 lg:px-8">
                  ...
                  >
                    ...
                    <div className="relative z-10 space-y-4 py-8 md:py-12">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs md:text-sm font-cinzel text-stone-600 tracking-wide leading-relaxed">Mr & Mrs Fernando</p>
                      </div>
                      <h3 className="text-5xl md:text-7xl font-playball text-theme-800 group-hover:scale-110 transition-transform duration-700 pt-6 drop-shadow-sm">Nimesh</h3>
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="relative z-10 space-y-4 py-8 md:py-12">
                      ...
                      <h3 className="text-5xl md:text-7xl font-playball text-theme-800 group-hover:scale-110 transition-transform duration-700 pt-6 drop-shadow-sm">Dhananji</h3>
                    </div>
                  </motion.div>
                </div>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-32 text-center w-full max-w-4xl px-4">
                    <div className="flex flex-col items-center flex-1">
                      ...
                      <p className="font-cinzel text-xl md:text-3xl text-theme-900 tracking-widest font-bold whitespace-nowrap">MONDAY, 14 DECEMBER</p>
                  ...
                  <div className="pt-8 w-full px-4">
                    <div className="relative inline-flex items-center justify-center w-full max-w-md mx-auto group">
                      ...
                      <p className="relative text-theme-800 bg-white shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] text-[9px] md:text-[11px] font-bold tracking-[0.3em] md:tracking-[0.4em] uppercase px-6 lg:px-10 py-4 lg:py-5 rounded-full border border-theme-200 flex items-center justify-center gap-4 w-full md:w-auto">
                        ...
                        <span className="whitespace-nowrap">Evening Celebration at 06:30 PM</span>
                        <span className="w-1.5 h-1.5 rotate-45 bg-theme-500 shrink-0" />
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid md:grid-cols-2 gap-16 md:gap-24 items-center">
                  ...
                  >
                    ...
                    <div className="pt-8 w-full md:w-auto">
                      <button
                        onClick={() => window.open('https://maps.app.goo.gl/MQ2D1aXojwez2m79A', '_blank')}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 w-full h-full scale-[1.2] group-hover:scale-[1.15] transition-transform duration-[2s]">
                      <iframe
                        src="https://maps.google.com/maps?q=Ramadia%20Grand,%20Moratuwa,%20Sri%20Lanka&t=&z=14&ie=UTF8&iwloc=&output=embed"
                        width="100%"
                        height="100%"
                        style={{ border: 0 }}
                        allowFullScreen
                  ...
                  <div className="w-full bg-white/5 backdrop-blur-md p-6 sm:p-8 md:p-12 rounded-[2rem] border border-white/10 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.5)]">
                    {rsvpStatus === 'success' ? (
                      <div className="text-center py-8">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-theme-200 text-stone-900 mb-6">
                          <Sparkles className="w-8 h-8" />
                        </div>
                        <h3 className="font-playball text-4xl text-theme-200 mb-4">Thank You!</h3>
                        <p className="text-stone-300 font-light tracking-wide">Your RSVP has been beautifully received.</p>
                      </div>
                    ) : (
            ...
            <div className="relative bg-[#fdfaf5]">
                ...
                <div className="container mx-auto px-4 max-w-4xl text-center relative z-10 w-full">
                  ...
                  >
                    ...
                    <div className="w-full max-w-2xl mx-auto bg-white p-6 sm:p-8 md:p-14 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] border border-theme-100/50 rounded-tr-[4rem] rounded-bl-[4rem] relative group">
                      ...
                      {wishesStatus === 'success' ? (
                        <div className="text-center py-12 relative z-10">
                          ...
                          <p className="text-stone-500 font-light tracking-wide">Your warm wishes mean the world to us.</p>
                        </div>
                      ) : (
                      <form className="space-y-8 text-left relative z-10" onSubmit={handleWishesSubmit}>
                        <div className="space-y-3">
                          ...
                          <input
                            type="text"
                            required
                            value={wishesData.name}
                            onChange={(e) => setWishesData({ ...wishesData, name: e.target.value })}
                        ...
                        <div className="space-y-3">
                          ...
                          <textarea
                            rows={4}
                            required
                            value={wishesData.message}
                            onChange={(e) => setWishesData({ ...wishesData, message: e.target.value })}
                    ...
                    <div className="mt-32 md:mt-48 space-y-6 flex flex-col items-center relative w-full">
                      ...
                      <h3 className="font-playball text-[3.2rem] sm:text-6xl md:text-8xl text-theme-900 relative z-10 drop-shadow-sm px-4 pt-4 leading-none">Nimesh & Dhananji</h3>
                      ...
                      <motion.img
                        ...
                        src={mandalaImage}
                        alt=""
                        className="relative z-10 mt-8 w-40 h-40 md:w-56 md:h-56 object-contain mix-blend-multiply drop-shadow-[0_12px_24px_rgba(188,121,36,0.2)]"
                      />
                    </div>
                  </motion.div>
                </div>
              ...
              <footer className="py-12 border-t border-theme-200/30 text-center relative z-10 space-y-3">
                <p className="text-[8px] md:text-[10px] uppercase tracking-[0.5em] text-stone-400 font-bold">
                  © 2026 Nimesh & Dhananji. <span className="hidden md:inline">|</span><br className="md:hidden block mt-2" /> All rights reserved.
                </p>
                <p className="text-[8px] md:text-[10px] tracking-[0.3em] text-stone-400">
                  Contact: <span className="text-theme-600 font-bold">Nimesh & Dhananji</span>
                </p>
              </footer>
            </div>
          </motion.div>
